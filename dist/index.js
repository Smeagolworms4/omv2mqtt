...
let argError = null;
if (!args.p)
    argError = 'omv-password as required';
if (!args.l)
    argError = 'omv-login as required';
if (!args.o)
    argError = 'omv-uri as required';
if (!args.m)
...
let omvDisableCheckHttps = parseInt(args['omv-disable-check-https'], 10);
isNaN(omvExposedNetworks) ? 0 : !!omvExposedNetworks;
let scanIterval = parseInt(args['scan-interval'], 10);
isNaN(scanIterval) || scanIterval < 1 ? 30 : scanIterval;
let loginIterval = parseInt(args['login-interval'], 10);
isNaN(loginIterval) || loginIterval < 1 ? 300 : loginIterval;
...
const main = async () => {
    try {
        let cookies = '';
        const requestOMV = async (body, connected = true) => {
        ...
        const publish = (path, data, sub = false) => {
            if (!sub) {
        ...
        const configHA = (type, id, name, path, extraConf = {}, expireAfter = true) => {
            if (haDiscovery) {
        ...
        const buttonHA = (type, id, name, path, extraConf = {}) => {
            if (haDiscovery) {
                publish(`${haPrefix}/${type}/${mqttPrefix}/${id.replace(/\W/gi, '_')}/config`, JSON.stringify({
                    uniq_id: mqttPrefix + '.' + id,
                    object_id: mqttPrefix + '.' + id,
        ...
        const updateServices = async () => {
            try {
                ...
                for (const service of json.response.data) {
                    ...
                    configHA('binary_sensor', `services.${service.name.toLowerCase()}`, service.title, `services/${service.name.toLowerCase()}`, {
        ...
        const updateSystem = async () => {
            try {
                ...
                const [infos, cpuTemp,] = await Promise.all([
                    callSystem(),
                ...
                configHA('sensor', `system.hostname`, 'Hostname', `system/hostname`, {
                    device: deviceSystem,
                    icon: 'mdi:web',
                });
                configHA('sensor', `system.kernel`, 'Kernel:', `system/kernel`, {
                    device: deviceSystem,
                });
                configHA('sensor', `system.version`, 'Version', `system/version`, {
                    device: deviceSystem,
                });
                configHA('sensor', `system.cpu_model_name`, 'CPU Model name', `system/cpu_model_name`, {
                    device: deviceSystem,
                });
                configHA('sensor', `system.cpu_usage`, 'CPU Usage', `system/cpu_usage`, {
                    device: deviceSystem,
                    unit_of_measurement: '%',
                    icon: 'mdi:speedometer',
                    state_class: 'measurement'
                });
                configHA('sensor', `system.memory`, 'Memory', `system/memory`, {
                    device: deviceSystem,
                    unit_of_measurement: '%',
                    icon: 'mdi:memory',
                    state_class: 'measurement'
                });
                configHA('sensor', `system.uptime`, 'Uptime', `system/uptime`, {
                    device: deviceSystem,
                    icon: 'mdi:clock-outline',
                    device_class: 'timestamp',
                });
                configHA('binary_sensor', `system.update_available`, 'Update available', `system/update_available`, {
                    device: deviceSystem,
                    icon: 'mdi:package',
                    device_class: 'update'
                });
                configHA('binary_sensor', `system.config_dirty`, 'Config dirty', `system/config_dirty`, {
                    device: deviceSystem,
                    icon: 'mdi:liquid-spot',
                    device_class: 'problem',
                });
                configHA('binary_sensor', `system.reboot_required`, 'Reboot required', `system/reboot_required`, {
                    device: deviceSystem,
                    icon: 'mdi:restart-alert',
                });
                configHA('sensor', `system.cpu_temperature`, 'CPU Temperature', `system/cpu_temperature`, {
                    device: deviceSystem,
                    icon: 'hass:thermometer',
                    unit_of_measurement: '°C',
                    state_class: 'measurement'
                });
                configHA('sensor', `system.last_refresh`, 'Last refresh', `system/last_refresh`, {
                    device: deviceSystem,
                    icon: 'mdi:clock-outline',
                    device_class: 'timestamp',
                }, false);
                buttonHA('button', `system.reboot`, 'Reboot', `system/reboot`, {
                    device: deviceSystem,
                    icon: 'mdi:restart',
                    payload_available: 'OK',
                    payload_not_available: 'FAILED',
                });
                buttonHA('button', `system.shutdown`, 'Shutdown', `system/shutdown`, {
                    device: deviceSystem,
                    icon: 'mdi:power',
                    payload_available: 'OK',
                    payload_not_available: 'FAILED',
            ...
            try {
                ...
                for (const network of json.response.data) {
                    if (omvExposedNetworks.indexOf(network.devicename) !== -1) {
                        ...
                        configHA('binary_sensor', `network.${network.devicename}.connection`, `Connection`, `networks/${network.devicename}/connection`, {
                            device,
                            icon: 'mdi:lan-connect'
                        });
                        configHA('sensor', `network.${network.devicename}.rx`, `RX`, `networks/${network.devicename}/rx`, {
                            device,
                        ...
                        configHA('sensor', `network.${network.devicename}.tx`, `TX`, `networks/${network.devicename}/tx`, {
                            device,
        ...
        const updateDisks = async () => {
            try {
                ...
                for (const [name, disk] of Object.entries(disks)) {
                    ...
                    configHA('sensor', `disk.${name}.smart`, 'SMART', `disks/${name}/smart`, {
                        device,
                        icon: 'mdi:harddisk',
                    });
                    configHA('sensor', `disk.${name}.temperature`, 'Temperature', `disks/${name}/temperature`, {
                        device,
                        icon: 'hass:thermometer',
                        unit_of_measurement: '°C',
                        state_class: 'measurement'
